...
function Step2Form({}: Properties): JSX.Element {
  ...
  const [selectedScenario, setScenario] = useState<Scenarios>(
    Scenarios.Warning1B,
  );

  if (Step2FormHeaderMessages[selectedScenario].type === 'error')
    return (
      <div id='step2form'>
            ...
            <div key={obj.lei} className='mb-2 flex flex-row gap-[0.9375rem]'>
              <div className='min-w-[6.25rem]'>
                <Notification
                  ...
                  message={success ? 'Approved' : 'Pending'}
                />
              </div>
              <Link
                className='border-b'
                href={'#'}
              >{`${obj.name} | ${obj.lei}`}</Link>
            </div>
          );
        })}
      </div>
